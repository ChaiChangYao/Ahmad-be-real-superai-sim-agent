"""Generated gravity stability test — default-robot-dog / gravity_stability."""
...
def load_sidecar_config() -> dict:
    ...
    return json.loads(ctx_path.read_text(encoding="utf-8"))from app.services.agentic.runtime.scene_helpers import add_mesh_entity, add_static_preview_mesh


def main() -> None:
    config = load_sidecar_config()
    start_run_metadata(config)
    write_manifest(config, {"warnings": ["Skeleton fallback preview. 53 mesh file(s) missing \u2014 not full visual simulation."]})
    frames: list[dict] = []
