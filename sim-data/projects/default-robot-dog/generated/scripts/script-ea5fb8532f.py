...
def load_sidecar_config() -> dict:
    ...
    return json.loads(ctx_path.read_text(encoding="utf-8"))from app.services.agentic.runtime.telemetry import create_telemetry_schema, summarize_telemetry
...
def main() -> None:
    ...
    write_manifest(config, {"warnings": ["Skeleton fallback preview. 53 mesh file(s) missing \u2014 not full visual simulation.", "53 meshes missing \u2014 skeleton joint sweep may still run."]})
    samples: list[dict] = []
    frames: list[dict] = []
