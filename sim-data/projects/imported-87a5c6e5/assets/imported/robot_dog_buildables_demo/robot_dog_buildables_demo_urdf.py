...
- Collision geometry: simplified local STL boxes/cylinders in meters.
  Physics collision is intentionally sparse: fixed/decorative body panels and
  joint housings are visual/inertial only to avoid self-contact impulses.
- Inertials: coarse primitive approximations suitable for early simulation,
  not measured mass properties.
- Joint limits include the zero joint state because most physics importers
  initialize unconstrained URDF joints at 0 unless an explicit pose is supplied.
"""
...
JOINT_DAMPING = 0.08
JOINT_FRICTION = 0.02
KNEE_LOWER_LIMIT = -2.65
KNEE_UPPER_LIMIT = 0.25
...
def body_link_specs() -> list[LinkSpec]:
    ...
    specs = [
        ...
        LinkSpec("front_sensor_panel_link", BASE_FRAME_M, (VisualMesh("torso_front_sensor_panel", "sensor_glass"), VisualMesh("front_depth_sensor_block", "sensor_glass")), None, 0.35, (0.06, 0.24, 0.13), (0.477, 0.0, 0.522 - BASE_FRAME_M[2])),
        LinkSpec("rear_service_panel_link", BASE_FRAME_M, (VisualMesh("torso_rear_service_panel", "anodized_black"),), None, 0.28, (0.05, 0.24, 0.13), (-0.477, 0.0, 0.518 - BASE_FRAME_M[2])),
        LinkSpec("internal_electronics_link", BASE_FRAME_M, (VisualMesh("internal_battery_tray", "anodized_black"), VisualMesh("internal_controller_mount", "anodized_black"), VisualMesh("battery_block", "battery_gray"), VisualMesh("controller_board_block", "pcb_green"), VisualMesh("imu_sensor_block", "pcb_green"), VisualMesh("power_distribution_block", "pcb_green"), VisualMesh("wire_harness_routes", "anodized_black")), None, 2.9, (0.43, 0.22, 0.12), (-0.04, 0.0, 0.49 - BASE_FRAME_M[2])),
        LinkSpec("payload_mount_link", BASE_FRAME_M, (VisualMesh("top_payload_mount_rail_left", "anodized_black"), VisualMesh("top_payload_mount_rail_right", "anodized_black"), VisualMesh("top_payload_block_placeholder", "payload_gray")), None, 1.1, (0.61, 0.18, 0.07), (0.03, 0.0, 0.675 - BASE_FRAME_M[2])),
    ]
    return specs
